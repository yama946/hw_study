...
```java
// 最大长度，长度偏移（偏移几个字节到长度说明），长度占用字节，长度之后还有几个字节到内容，剥离字节数
...
* 魔数，用来在第一时间判定是否是无效数据包，是否是可以被解析的数据
* 版本号，可以支持协议的升级
...
* netty中的handler，只要添加@Sharable注解，表示是线程安全的，创建一个，可以被多个eventLoop使用。
* 当 handler 不保存状态时，也就是无状态的，就可以安全地在多线程下被共享
* 但要注意对于编解码器类，不能继承 ByteToMessageCodec 或 CombinedChannelDuplexHandler 作为父类，他们的构造方法对 @Sharable 有限制
* 如果能确保编解码器不会保存状态，线程安全的，可以继承 MessageToMessageCodec 父类，再自定义编解码器上添加@Sharable 注解
