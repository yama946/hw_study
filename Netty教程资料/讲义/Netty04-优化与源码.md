...
* SO_TIMEOUT 主要用在阻塞 IO（传统BIO的Socket中），阻塞 IO 中 accept，read 等都是无限等待的，如果不希望永远阻塞，使用它调整超时时间
...
* syns queue：半连接队列，存放没有完成三次握手的连接信息
* accept queue：全连接队列，存放完成三次握手的连接信息
* 为什么需要全连接队列？因为服务器的accept的能力是有限的，将完成三次握手的连接信息先存放在队列中，进行有条不紊的建立连接。
...
* accept queue - 全连接队列
  ...
  * 如果系统是linux，除了需要设置程序中参数，还要修改Linux中配置文件的参数；如果系统是windows会依照程序中的设置生效。
  * 如果 accpet queue 队列满了，表示服务器处理连接的能力达到上限，server 将发送一个拒绝连接的错误信息到 client

netty 中

可以通过  option(ChannelOption.SO_BACKLOG, 值) 来设置大小，nio中可以通过bind(port,blacklog)设置大小。
...
* SO_SNDBUF 属于 SocketChannal 参数（不建议调整）
* SO_RCVBUF 既可用于 SocketChannal 参数，也可以用于 ServerSocketChannal 参数（建议设置到 ServerSocketChannal 上）（不建议调整）
