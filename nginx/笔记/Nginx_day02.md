...
注意 ~后面不能加空格，括号可以取值，通过$1获取第一个括号对应的内容。
```
...
^~: 用于不包含正则表达式的uri前，功能和不加符号的一致，唯一不同的是，如果模式匹配，那么就停止搜索其他模式了。如果不添加会向下搜索，如果存在匹配的就直接返回。
...
所选择的值可以从mime.types文件中进行查找，也可以使用"*"代表所有，但是星号生产环境中并不常用，因为有些视频资源都是高度压缩的，再次使用压缩会浪费cpu资源。
...
```
http{
	gzip_comp_level 6;//9的压缩效果并不比6要明显
}
```
...
regex:根据客户端的浏览器标志(user-agent)来设置，支持使用正则表达式。指定的浏览器标志不使用Gzip.该指令一般是用来排除一些明显不支持Gzip的浏览器，否则就会出现乱码。
...
| 语法   | **gzip_static** on \| off \| always;（always不管浏览器是否支持都进行压缩） |
| ------ | ------------------------------------------------------------ |
| 默认值 | gzip_static off;                                             |
| 位置   | http、server、location                                       |
...
(3)再次访问`http://192.168.200.133/jquery.js`，此时gzip_static会查找一个与请求文件同名的.gz压缩文件返回。
...
| 变量               | 说明                                                         |
| ------------------ | ------------------------------------------------------------ |
| $args              | 变量中存放了请求URL中的请求指令。比如http://192.168.200.133:8080?arg1=value1&args2=value2中的"arg1=value1&arg2=value2"，功能和query_string一样 |
| $http_user_agent   | 变量存储的是用户访问服务的代理信息(如果通过浏览器访问，记录的是浏览器的相关版本信息) |
| $host              | 变量存储的是访问服务器的server_name值                        |
| $document_uri      | 变量存储的是当前访问地址的URI。比如http://192.168.200.133/server?id=10&name=zhangsan中的"/server"，功能和uri一样 |
